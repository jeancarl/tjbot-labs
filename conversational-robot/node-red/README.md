...
20. Here's an example of what the `response` object looks like.
    ...
    Add a ![](assets/nodes/change.png) node to move the response into the `msg.payload` property. Set `msg.payload` to the expression `$join(msg.response.object.output.text, " ")`.
